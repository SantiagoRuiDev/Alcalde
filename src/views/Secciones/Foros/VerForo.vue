       ...
       <div class="col px-0">
         <div class="px-4 py-5 chat-box bg-white">
           ...
           <div class="media w-50 mb-3" v-for="mensaje in mensajes" :key="mensaje.id">
            ...
            <small class="text-muted mx-3">{{ mensaje.nombre }}</small>
             <div class="media-body ml-3 mt-2 d-flex gap-2 align-items-center">
               <div class="bg-light rounded py-2 px-3 mb-2" v-if="mensaje.mensaje != ''">
                ...
                <button @click="replyMessage(mensaje.id)" type="button" class="border-0 bg-transparent">
                  <span class="material-symbols-outlined" @click="replyMessage(mensaje.id)"> reply </span>
                </button>
                <button type="button" class="border-0 bg-transparent d-flex items-center mb-1 gap-2" v-if="mensaje.likes.length > 0">
                  {{ mensaje.likes.length }}
                  <span class="material-symbols-outlined" @click="likeMessage(mensaje.id)"> thumb_up </span>
                </button>
                ...
                <button @click="strikeUsuario(mensaje.id_usuario)" type="button" class="border-0 bg-transparent">
                  <span class="material-symbols-outlined" @click="strikeUsuario(mensaje.id_usuario)"> warning </span>
                </button>
              </div>
            ...
            <div class="" v-if="mensaje.replicas.length > 0">
              <div class="" v-for="replica in mensaje.replicas" :key="replica.id">
                <div class="replica-box">
                  <img src="https://therichpost.com/wp-content/uploads/2020/06/avatar3.png" alt="user" width="30" class="rounded-circle">
                  <small class="text-muted mx-3">{{ replica.nombre }} : Replica a {{ mensaje.nombre }}</small>
                  <div class="media-body ml-3 mt-2 d-flex gap-2 align-items-center">
                    <div class="bg-light rounded py-2 px-3 mb-2" v-if="replica.mensaje != ''">
                      <p class="text-small mb-0 text-muted">{{replica.mensaje}}</p>
                    </div>
                    <button @click="deleteReply(replica.id)" type="button" class="border-0 bg-transparent">
                      <span class="material-symbols-outlined" @click="deleteReply(replica.id)"> delete </span>
                    </button>
                    <button @click="strikeUsuario(replica.id_usuario)" type="button" class="border-0 bg-transparent">
                      <span class="material-symbols-outlined" @click="strikeUsuario(replica.id_usuario)"> warning </span>
                    </button>
                  </div>

                        
                  <div v-if="replica.imagen != ''">
                    <img :src="replica.imagen" alt="Chat Message Image" class="reply-image"/>
                  </div>
                </div>
              </div>
            </div>
         ...
         <form action="#" class="bg-light" v-if="reply">
            <button type="button" class="border-0 bg-transparent my-2">
              <span class="material-symbols-outlined" @click="closeReply()"> close </span>
            </button>
           ...
           <div class="input-group">
             <input type="text" v-model="mensaje" placeholder="Replica a este mensaje" aria-describedby="button-addon2" class="form-control rounded-0 border-0 py-4 bg-light">
...
export default {
    ...
    methods: {
        ...
        strikeUsuario(id){
          this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.axios.post(this.URL_STRIKES + id, {}, {
                        headers: {
                            'x-access-token': this.$store.getters.getUserToken,
                        }
                     }).then(response => {
        ...
        likeMessage(mensaje){
          this.axios.post(this.URL_LIKE + this.$route.params.id, {id_mensaje: mensaje}, {
                    headers: {
                        'x-access-token': this.$store.getters.getUserToken
                    }
                })
        ...
        deleteReply(id){
          this.axios.post(this.URL_ELIMINAR_REPLY + id, {}, {
                    headers: {
                        'x-access-token': this.$store.getters.getUserToken
                    }
                })
        ...
        ordenarMensajes(){
          ...
          this.mensajes.forEach(mensaje => {
            mapNewMensajes.push({
              ...
              replicas: this.replicas.filter(replica => replica.id_mensaje == mensaje.id)
            })
          });
        ...
        crearMensaje(){
          if(this.reply){
            ...
            if(this.mensaje.trim() != "" || this.selectedFile != null){
                this.axios.post(this.URL_REPLY + this.$route.params.id, this.data, {
                    headers: {
                        'x-access-token': this.$store.getters.getUserToken
                    }
                })
